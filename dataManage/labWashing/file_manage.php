...
include '../db.php';
 ...
 if(!empty($_FILES))
 {
    ...
    for ($i=0; $i < $total; $i++) {
      ...
      if (file_exists($target_file) && ($target_file != $target_dir)) {
        ...
        if ($_FILES["files"]["size"][$i] > 30000000) {
          ...
          $check_query = "SELECT * FROM filewashtbl WHERE sampleNo = '".$sampleNo."' 
          AND fileName = '".$file_raw_name."'";
          $result = mysqli_query($connect, $check_query);
          $check_query_row_number = mysqli_num_rows($result);
          //파일 정보를 데이터베이스에 저장
          if($check_query_row_number == 0){
              $query = "INSERT INTO filewashtbl (
                sampleNo, fileName, fileType, fileSize) VALUES (
                  '".$sampleNo."', '".$file_raw_name."', '".$file_type."', ".$file_size.")";
  ...
  $sql = "DELETE FROM filewashtbl WHERE fileName = '".$sqlFileName."'";
