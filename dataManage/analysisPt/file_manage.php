...
include '../db.php';
 ...
 if(!empty($_FILES))
 {
      ...
      $pasteNo = $_POST["file-pasteNo"];
      $timeAging = $_POST["file-timeAging"];
  ...
  for ($i=0; $i < $total; $i++){
    ...
    if (file_exists($target_file) && ($target_file != $target_dir)) {
      ...
      if ($_FILES["files"]["size"][$i] > 30000000) {
        ...
        $check_query = "SELECT * FROM filepttbl WHERE pasteNo = '".$pasteNo."' 
        AND timeAging = '".$timeAging."' AND fileName = '".$file_raw_name."'";
        $result = mysqli_query($connect, $check_query);
        $check_query_row_number = mysqli_num_rows($result);
        //파일 정보를 데이터베이스에 저장
        if($check_query_row_number == 0){
            $query = "INSERT INTO filepttbl (
              pasteNo, timeAging, fileName, fileType, fileSize) VALUES (
                '".$pasteNo."', '".$timeAging."', '".$file_raw_name."', '".$file_type."', ".$file_size.")";
  ...
  $sql = "DELETE FROM filepttbl WHERE fileName = '".$sqlFileName."'";
