...
include '../db.php';
 ...
 if(!empty($_FILES))
 {
      ...
      $lotNo = $_POST["file-lotNo"];
      $classPost = $_POST["file-classPost"];
...
for ($i=0; $i < $total; $i++){
  ...
  if (file_exists($target_file) && ($target_file != $target_dir)) {
    ...
    if ($_FILES["files"]["size"][$i] > 30000000) {
      ...
      $check_query = "SELECT * FROM filepwmasstbl WHERE lotNo = '".$lotNo."' 
      AND classPost = '".$classPost."' AND fileName = '".$file_raw_name."'";
      $result = mysqli_query($connect, $check_query);
      $check_query_row_number = mysqli_num_rows($result);
      //파일 정보를 데이터베이스에 저장
      if($check_query_row_number == 0){
          $query = "INSERT INTO filepwmasstbl (
            lotNo, classPost, fileName, fileType, fileSize) VALUES (
              '".$lotNo."', '".$classPost."', '".$file_raw_name."', '".$file_type."', ".$file_size.")";
  ...
  $sql = "DELETE FROM filepwmasstbl WHERE fileName = '".$sqlFileName."'";
